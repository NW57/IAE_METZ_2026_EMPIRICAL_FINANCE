...
This project successfully developed a Python-based tool for strategic portfolio optimization, adhering strictly to the constraints outlined in a detailed Investment Policy Statement (IPS). Building upon the foundational principles of Modern Portfolio Theory (MPT), the primary objective was to maximize risk-adjusted performance while ensuring robustness against market uncertainties.

The methodology encompassed several critical steps: a rigorous data preparation phase involving the selection of assets based on historical Sharpe ratios (including key large-cap equities and gold), followed by the implementation of a constrained Markowitz mean-variance optimization model. This model incorporated explicit IPS constraints on volatility, maximum drawdown, Value-at-Risk, and asset allocation limits (no short-selling, individual asset weight between 0% and 30%, 10-15 assets, and a fully invested portfolio).

The analysis provided a comprehensive dashboard comparing the optimized portfolio against an equally-weighted portfolio and a passive benchmark (80% MSCI World / 20% Gold). While the optimal portfolio demonstrated superior Sharpe ratios and effective risk management within the specified IPS limits (e.g., volatility and VaR), the stress testing and Monte Carlo simulations highlighted its performance under adverse conditions and provided forward-looking perspectives on potential value trajectories.

Furthermore, the integration of machine learning models for adaptive allocation demonstrated the potential for dynamic adjustments based on evolving market conditions, enhancing the portfolio's resilience and adaptability. Visualizations, including cumulative returns comparisons, efficient frontier plots, and Monte Carlo simulation outcomes, underscored the practical advantages of a quantitative approach to portfolio management. The project affirms Python's capabilities in handling complex financial data, performing advanced optimizations, and providing actionable insights for informed investment decision-making.